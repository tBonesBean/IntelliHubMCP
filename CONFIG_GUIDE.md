...
```json
{
	"servers": {
		"intellihub_stdio": {
			...
			"command": "python",
			"args": [
				"/absolute/path/to/intellihub_tool/stdio_server.py"
			]
		}
	},
	"inputs": []
	...
	"servers": {
		"intellihub_stdio": {
			...
			"args": [
				...
				"--host", "localhost",  // ❌ stdio doesn't accept arguments
				"--port", "8002",       // ❌ stdio doesn't accept arguments
...
3. **Test stdio server:**
   ```bash
   cd intellihub_tool
   echo '{"jsonrpc":"2.0","method":"initialize","id":1,"params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"1.0.0"}}}' | python stdio_server.py
   ```
